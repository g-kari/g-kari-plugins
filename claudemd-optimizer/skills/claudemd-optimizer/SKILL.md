...
- 「コミット前にlintを実行すること」→ PreToolUse hook（matcher: `Bash` でgit commitコマンドを検知）
- 「ビルドが通ることを確認すること」→ PostToolUse hook（matcher: `Edit` or `Write`）
- 「フォーマットを整えること」→ PostToolUse hook（matcher: `Edit` or `Write`）
- 「型チェックを実行すること」→ PostToolUse hook（matcher: `Edit` or `Write`）
