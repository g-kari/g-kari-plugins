...
AI（Claude Code）が起票・コメントする Issue / PR は、起票主体を一目で識別できるよう、以下のフォーマットを **必ず** 使用する。

**Issue 本文の先頭:**

```markdown
> 🤖 **AI 起票 (Claude Code)** — このIssueはコードベース精査または振り返りから AI が自動起票しました。設計判断・優先度はユーザーが決定してください。
...
| 状況 | バナーを付ける？ |
|---|---|
| 観点別 explore agent から派生した起票 | ✅ 付ける |
| retrospective から派生した起票 | ✅ 付ける |
| AI による調査・実装計画コメント | ✅ 付ける |
| ユーザーが直接書いた Issue | ❌ 付けない |
| ユーザー指示の代筆コメント（「このコメントを送って」と頼まれた場合） | ❌ 付けない |

**Why:** AI と人間の意思決定を区別することで、レビュー・優先度判断・トラブル時の責任所在が明確になる。過去にユーザーから「Issue 描くとき、俺が書いたのか、AI が書いたのか分からん」と指摘された経緯あり。
...
gh issue list --author "@me" --state open --json number,title,body
# 本文先頭に "🤖 AI 起票" バナーがないものが遡及対象
```
